...
@blog_routes.get(
    "/", response_model=List[BlogResponseSchema], status_code=status.HTTP_200_OK
)
...
@blog_routes.get(
    "/{id}", response_model=BlogResponseSchema, status_code=status.HTTP_200_OK
)
...
@blog_routes.put(
    "/{id}", response_model=BlogResponseSchema, status_code=status.HTTP_201_CREATED
)
def update_blog(
    id: uuid.UUID,
...
@blog_routes.delete(
    "/blog/{id}", response_model=None, status_code=status.HTTP_204_NO_CONTENT
)
def delete_blog(
    id: uuid.UUID,
    db: Session = Depends(get_db),
