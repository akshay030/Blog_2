...
def edit_profile(body: UpdateProfileSchema, db: Session, user: UserModel):
    ...
    db.commit()
    redis_client.delete(f"profile:{user.id}")
    db.refresh(profile)

    return profile


def get_my_profile(db: Session, user: UserModel):
    ...
    redis_client.set(cache_key, json.dumps(profile_data), ex=300)
