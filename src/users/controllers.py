...
def register(body: UserSchema, db: Session):
    if body.role == UserRole.ADMIN:
        ...
        admin_exists = (
            db.query(UserModel).filter(UserModel.role == UserRole.ADMIN).first()
        )
    ...
    new_user = UserModel(
        name=body.name, email=body.email, hash_password=hash_password, role=body.role
    )
    db.add(new_user)
    db.commit()
    db.refresh(new_user)

    profile = ProfileModel(bio="", user_id=new_user.id)
...
def login(body: LoginSchema, db: Session):
    ...
    redis_client.set(f"session:{user.id}", token, ex=1800)
    create_audit_log(db=db, user_id=user.id, action="LOGIN", resource="USER")
