...
@user_routes.post(
    "/register", response_model=UserResponseSchema, status_code=status.HTTP_200_OK
)
...
@user_routes.get(
    "/is_auth", response_model=IsAuthResponseSchema, status_code=status.HTTP_200_OK
)
...
def logout(user: UserModel = Depends(is_authenticated)):
